...
function setupAccessGuard(router: Router) {
  router.beforeEach(async (to, from) => {
    ...
    // 预加载地区树数据并缓存到 localStorage（先取缓存，没有再请求 API）
    // try {
    //   const areaTreeResult = await getAreaTree({ levels: [0, 1, 2] });
    //   if (areaTreeResult?.length) {
    //     localStorage.setItem('local_storage_data-areaTree', JSON.stringify(areaTreeResult));
    //   }
    // } catch (error) {
    //   console.error('Failed to load area tree data:', error);
    //   localStorage.removeItem('local_storage_data-areaTree');
    // }
    ...
    const redirectPath = (from.query.redirect ??
      (to.path === '/' || to.path === '' || to.path === preferences.app.defaultHomePath
