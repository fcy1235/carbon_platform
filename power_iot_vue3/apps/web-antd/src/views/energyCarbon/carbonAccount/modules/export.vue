...
import { exportAccounting } from '#/api/energyCarbon/carbonAccount';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.activityNameCode) params.activityNameCode = queryParams.activityNameCode;
        // 处理行政区划级联选择器值
      ...
      const blob = await exportAccounting(params);
      ...
      if (blob) {
        ...
        a.href = url;
        a.download = `碳排放核算数据_${Date.now()}.xlsx`;
