...
import { exportGasInfo } from '#/api/energyCarbon/gasInfo';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.gasCode) params.gasCode = queryParams.gasCode;
        if (queryParams.gasName) params.gasName = queryParams.gasName;
        if (queryParams.category) params.category = queryParams.category;
      }

      const blob = (await exportGasInfo(params as any)) as Blob;
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = '气体信息数据.xlsx';
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出气体信息数据吗？导出格式为 Excel</div>
