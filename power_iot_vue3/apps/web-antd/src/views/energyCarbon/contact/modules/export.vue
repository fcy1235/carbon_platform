...
import { exportContact } from '#/api/energyCarbon/contact';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.company) params.company = queryParams.company;
      }

      const blob = await exportContact(params);
      ...
      if (blob) {
        ...
        a.href = url;
        a.download = `联系人数据_${Date.now()}.xlsx`;
