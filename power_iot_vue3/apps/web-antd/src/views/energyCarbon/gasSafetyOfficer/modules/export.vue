...
import { exportGasSafetyOfficer } from '#/api/energyCarbon/gasSafetyOfficer';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.qualificationNo) params.qualificationNo = queryParams.qualificationNo;
      ...
      const blob = (await exportGasSafetyOfficer(params as any)) as Blob;
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = '燃气安全员数据.xlsx';
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出燃气安全员数据吗？导出格式为 Excel</div>
