...
import { exportGasCoordinator } from '#/api/energyCarbon/gasCoordinator';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.staffStatus) params.staffStatus = queryParams.staffStatus;
        if (queryParams.isVillageCommitteeMember) params.isVillageCommitteeMember = queryParams.isVillageCommitteeMember;
        if (queryParams.areaCode) params.areaCode = queryParams.areaCode;
      }

      const blob = (await exportGasCoordinator(params as any)) as Blob;
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = '农村气代煤协管员数据.xlsx';
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出农村气代煤协管员数据吗？导出格式为 Excel</div>
