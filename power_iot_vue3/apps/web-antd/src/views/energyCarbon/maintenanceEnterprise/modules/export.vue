...
import { exportMaintenanceEnterprise } from '#/api/energyCarbon/maintenanceEnterprise';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.countyCode) params.countyCode = queryParams.countyCode;
        if (queryParams.enterpriseName) params.enterpriseName = queryParams.enterpriseName;
        if (queryParams.unifiedSocialCreditCode) params.unifiedSocialCreditCode = queryParams.unifiedSocialCreditCode;
        if (queryParams.serviceType) params.serviceType = queryParams.serviceType;
        if (queryParams.serviceStatus) params.serviceStatus = queryParams.serviceStatus;
      }

      const blob = (await exportMaintenanceEnterprise(params as any)) as Blob;
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = '维保企业数据.xlsx';
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出维保企业数据吗？导出格式为 Excel</div>
