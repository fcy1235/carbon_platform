...
import { exportFactorLib } from '#/api/energyCarbon/factorLib';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.factorName) params.factorName = queryParams.factorName;
        if (queryParams.emissionSource) params.emissionSource = queryParams.emissionSource;
      }

      const blob = await exportFactorLib(params);
      ...
      if (blob) {
        ...
        a.href = url;
        a.download = `排放因子库数据_${Date.now()}.xlsx`;
