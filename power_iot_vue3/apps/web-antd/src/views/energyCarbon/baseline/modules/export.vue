...
import { exportBaseline } from '#/api/energyCarbon/baseline';
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    try {
      ...
      const blob = (await exportBaseline(params as any)) as Blob;
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = '基准线碳排放强度数据.xlsx';
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出基准线碳排放强度数据吗？导出格式为 Excel</div>
