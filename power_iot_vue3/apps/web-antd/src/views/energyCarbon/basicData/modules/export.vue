...
import { exportUserInfo } from '#/api/energyCarbon/basicData';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.idCard) params.idCard = queryParams.idCard;
        ...
        if (queryParams.townCode) params.townCode = queryParams.townCode;
        if (queryParams.villageCode) params.villageCode = queryParams.villageCode;
        if (queryParams.reformType) params.reformType = queryParams.reformType;
        if (queryParams.reformMode) params.reformMode = queryParams.reformMode;
        if (queryParams.useStatus) params.useStatus = queryParams.useStatus;
      }

      const blob = (await exportUserInfo(params as any)) as Blob;
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出用户数据吗？导出格式为 Excel</div>
