...
import { exportGasData } from '#/api/energyCarbon/gas';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.gasId) params.gasId = queryParams.gasId;
      }

      const blob = (await exportGasData(params as any)) as Blob;
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = '燃气数据.xlsx';
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出燃气数据吗？导出格式为 Excel</div>
