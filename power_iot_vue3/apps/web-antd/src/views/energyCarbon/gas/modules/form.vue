  ...
  schema: [
    {
      ...
      component: 'Select',
      componentProps: {
        options: getDictOptions(DICT_TYPE.ENERGY_CBON_GAS_SOURCE, 'string'),
        placeholder: '请选择数据来源',
        allowClear: true,
        disabled: true,
      },
...
const [HistoryGrid, historyGridApi] = useVbenVxeGrid({
  gridOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async () => {
          if (!formData.value?.gasId) {
            return { list: [], total: 0 };
          }
          const result: any = await getGasDetailData(formData.value.gasId);
...
async function handleConfirmDeleteHistory() {
  ...
  try {
    await deleteGasData(selectedHistoryKeys.value);
...
<template>
  <Modal>
    <div class="mx-4">
      <h4 class="text-base font-semibold mb-4">基础燃气数据</h4>
      <Form />

      <h4 class="text-base font-semibold mb-4 mt-6">历史燃气数据</h4>
      <div class="mb-2">
