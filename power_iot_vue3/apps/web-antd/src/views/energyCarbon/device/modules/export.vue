...
import { exportDevice } from '#/api/energyCarbon/device';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.deviceName) params.deviceName = queryParams.deviceName;
        if (queryParams.deviceType) params.deviceType = queryParams.deviceType;
        if (queryParams.manufacturer) params.manufacturer = queryParams.manufacturer;
        if (queryParams.reformType) params.reformType = queryParams.reformType;
        if (queryParams.status) params.status = queryParams.status;
        // 处理行政区划级联选择器值
        if (queryParams.division && queryParams.division.length > 0) {
          params.provinceCode = queryParams.division[0];
          if (queryParams.division.length > 1) {
            params.cityCode = queryParams.division[1];
          }
          if (queryParams.division.length > 2) {
            params.districtCode = queryParams.division[2];
          }
        }
      }

      const blob = await exportDevice(params);
      ...
      if (blob) {
        ...
        a.href = url;
        a.download = `设备管理数据_${Date.now()}.xlsx`;
