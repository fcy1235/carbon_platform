...
import {
  getDeviceImportTemplate,
  importDeviceInfo,
  type DeviceImportResult,
} from '#/api/energyCarbon/device/import';
...
async function handleDownloadTemplate() {
  try {
    const blob = (await getDeviceImportTemplate()) as Blob;
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    modalApi.lock();
    try {
      const data = await importDeviceInfo(file);
  ...
  <Modal>
    <div>
      <div class="import-description">
        ...
        格式，必填字段不可为空。模板中「燃气表类型」填 1/2/3/4，「安全装置配备情况」填
        1/2/3，「改造类别」填「煤改气」。模板第 2 行及以后请勿填写示例数据。
      </div>
