...
import {
  getSubsidyImportTemplate,
  importSubsidy,
} from '#/api/energyCarbon/subsidy';
...
async function handleDownloadTemplate() {
  try {
    const blob = (await getSubsidyImportTemplate()) as Blob;
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = '补贴导入模板.xlsx';
  ...
  <Modal>
    <div>
      <div class="import-description">
        导入说明：请下载模板并按规范填写数据，仅支持 Excel 格式，必填字段不可为空。
      </div>
