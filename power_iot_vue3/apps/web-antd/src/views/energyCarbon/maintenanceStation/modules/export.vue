...
import { exportMaintenanceStation } from '#/api/energyCarbon/maintenanceStation';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.stationName) params.stationName = queryParams.stationName;
        if (queryParams.serviceType) params.serviceType = queryParams.serviceType;
        if (queryParams.businessStatus) params.businessStatus = queryParams.businessStatus;
      }

      const blob = (await exportMaintenanceStation(params as any)) as Blob;
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = '维保网点数据.xlsx';
...
<template>
  <Modal>
    <div>
      <!-- 导出说明 -->
      <div class="export-description">确认导出维保网点数据吗？导出格式为 Excel</div>
