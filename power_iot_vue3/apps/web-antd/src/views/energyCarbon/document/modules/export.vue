...
import { exportDocument } from '#/api/energyCarbon/document';
  ...
  async onConfirm() {
    ...
    try {
      ...
      if (exportType.value === 'selected') {
        ...
        if (queryParams.docTitle) params.docTitle = queryParams.docTitle;
        if (queryParams.uploader) params.uploader = queryParams.uploader;
      }

      const blob = await exportDocument(params);
      ...
      if (blob) {
        ...
        a.href = url;
        a.download = `文档管理数据_${Date.now()}.xlsx`;
