package cn.iocoder.power.gateway.filter.security;

import cn.hutool.core.util.StrUtil;
import cn.iocoder.power.framework.common.core.KeyValue;
...
import cn.iocoder.power.gateway.util.SecurityFrameworkUtils;
import cn.iocoder.power.gateway.util.WebFrameworkUtils;
import cn.iocoder.power.framework.common.biz.system.oauth2.OAuth2TokenCommonApi;
import cn.iocoder.power.framework.common.biz.system.oauth2.dto.OAuth2AccessTokenCheckRespDTO;
...
import static cn.iocoder.power.framework.common.util.cache.CacheUtils.buildAsyncReloadingCache;
...
public class TokenAuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        log.info("[filter][uri({} {}) 开始 Token 认证][tenantId: {}]",
                exchange.getRequest().getURI(), exchange.getRequest().getMethod(), WebFrameworkUtils.getTenantId(exchange));
        return getLoginUser(exchange, token).defaultIfEmpty(LOGIN_USER_EMPTY).flatMap(user -> {
            ...
            if (user == LOGIN_USER_EMPTY || // 下面 expiresTime 的判断，为了解决 token 实际已经过期的情况
                ...
                log.warn("[filter][uri({} {}) Token 认证失败或已过期][tenantId: {}]",
                        finalExchange.getRequest().getURI(), finalExchange.getRequest().getMethod(), WebFrameworkUtils.getTenantId(finalExchange));
                return chain.filter(finalExchange);
            }

            // 2.1 有用户，则设置登录用户
            log.info("[filter][uri({} {}) Token 认证成功][userId: {}][tenantId: {}]",
                    finalExchange.getRequest().getURI(), finalExchange.getRequest().getMethod(), user.getId(), user.getTenantId());
        ...
        }).doOnError(ex -> log.error("[filter][uri({} {}) Token 认证发生异常][tenantId: {}][异常类型: {}]",
                finalExchange.getRequest().getURI(), finalExchange.getRequest().getMethod(),
                WebFrameworkUtils.getTenantId(finalExchange), ex.getClass().getName(), ex));
    }
    ...
    private Mono<LoginUser> getLoginUser(ServerWebExchange exchange, String token) {
        ...
        if (localUser != null) {
            log.info("[getLoginUser][tenantId({}) token 缓存命中][userId: {}]", tenantId, localUser.getId());
            return Mono.just(localUser);
        }

        // 缓存不存在，则请求远程服务
        log.info("[getLoginUser][tenantId({}) token 缓存未命中，开始远程校验]", tenantId);
        return checkAccessToken(tenantId, token).flatMap((Function<String, Mono<LoginUser>>) body -> {
            ...
            if (remoteUser != null) {
                ...
                log.info("[getLoginUser][tenantId({}) 远程校验成功][userId: {}]", tenantId, remoteUser.getId());
                return Mono.just(remoteUser);
            }
            log.warn("[getLoginUser][tenantId({}) 远程校验返回无效用户]", tenantId);
            return Mono.empty();
        });
    }

    private Mono<String> checkAccessToken(Long tenantId, String token) {
        log.info("[checkAccessToken][tenantId({}) 开始调用鉴权服务]", tenantId);
        return webClient.get()
                ...
                .doOnSuccess(body -> log.info("[checkAccessToken][tenantId({}) 鉴权服务调用成功][响应长度: {}]",
                        tenantId, body != null ? body.length() : 0))
                .doOnError(ex -> log.error("[checkAccessToken][tenantId({}) 鉴权服务调用失败][异常类型: {}][异常消息: {}]",
                        tenantId, ex.getClass().getName(), ex.getMessage()));
    }
    ...
    private LoginUser buildUser(String body) {
        ...
        if (result == null) {
            log.warn("[buildUser][鉴权响应解析为 null][body: {}]", body);
            return null;
        }
        if (result.isError()) {
            log.warn("[buildUser][鉴权响应业务错误][code: {}][msg: {}]", result.getCode(), result.getMsg());
