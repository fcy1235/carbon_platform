package cn.iocoder.power.gateway.filter.grey;
...
public class GrayReactiveLoadBalancerClientFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 修改 by root源码：将 lb 替换成 grayLb，表示灰度负载均衡
        ...
        log.info("[filter][uri({} {}) 开始灰度负载均衡][serviceId: {}]",
                exchange.getRequest().getURI(), exchange.getRequest().getMethod(), serviceId);
        ...
        return choose(lbRequest, serviceId, supportedLifecycleProcessors).doOnNext(response -> {
                    ...
                    if (!response.hasServer()) {
                        log.error("[filter][uri({} {}) 负载均衡未找到可用实例][serviceId: {}]",
                                exchange.getRequest().getURI(), exchange.getRequest().getMethod(), serviceId);
                    ...
                    log.info("[filter][uri({} {}) 负载均衡选择实例成功][serviceId: {}][instance: {}:{}]",
                            exchange.getRequest().getURI(), exchange.getRequest().getMethod(),
                            serviceId, retrievedInstance.getHost(), retrievedInstance.getPort());
                ...
                .doOnError(throwable -> {
                    log.error("[filter][uri({} {}) 负载均衡或转发异常][serviceId: {}][异常类型: {}][异常消息: {}]",
                            exchange.getRequest().getURI(), exchange.getRequest().getMethod(),
                            serviceId, throwable.getClass().getName(), throwable.getMessage());
                    supportedLifecycleProcessors.forEach(lifecycle -> lifecycle
    ...
    private Mono<Response<ServiceInstance>> choose(Request<RequestDataContext> lbRequest, String serviceId,
        ...
        // 修改 by root源码：直接创建 GrayLoadBalancer 对象
