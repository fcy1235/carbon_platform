package cn.iocoder.power.gateway.handler;
 ...
 *
 * 在功能上，和 power-spring-boot-starter-web 的 GlobalExceptionHandler 类是一致的
 *
 * 
 */
@Component
...
public class GlobalExceptionHandler implements ErrorWebExceptionHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, Throwable ex) {
        ...
        if (response.isCommitted()) {
            log.error("[handle][uri({} {}) 响应已提交，无法处理异常][异常类型: {}][异常消息: {}]",
                    request.getURI(), request.getMethod(), ex.getClass().getName(), ex.getMessage(), ex);
